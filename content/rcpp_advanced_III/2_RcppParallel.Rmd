...
```
defined in the `RcppParallel` namespace. The first two arguments define the beginning and end of the for loop, the second is an object of type `Worker` (which we will discuss below) and the last argument defines determined the minimal chunk size for parallelization. That is, if we set `grainSize = 10`, then each thread will perform at least 10 iterations of the for loop. In the code above, an argument of type `ErfVec` is passed to `RcppParallel`, in the `Worker` argument. Objects of type `ErfVec` are data structures which inherit the `RcppParallel::Worker` type. The structure are two elements or members:
```{r engine='Rcpp', eval = FALSE}
const RVector<double> in;
RVector<double> out;
```
...
```
which will be called by `parallelFor` on a section of the input vector delimited by `being` and `end`. 

Note that `parallelFor` is itself an wrapper around a call to:
```{r engine='Rcpp', eval = FALSE}
tbb::parallel_for(tbb::blocked_range<size_t>(begin, end, grainSize), 
                     tbbWorker);
```
which is defined in Intel's [Threading Building Blocks (TBB)](https://github.com/oneapi-src/oneTBB) C++ library, which is shipped with `RcppParallel`. TBB is available on Windows, OS X, Linux, and Solaris x86, while on other platform will `parallelFor` will fall back on:
```{r engine='Rcpp', eval = FALSE}
...
```
So on a 4 cores machine we are over 3 times faster. Let's try to understand how this woks. The `dnormPar` function is relatively similar to the `erfPar` example, described above. In particular, `std::accumulate` has been substituted by 
```{r engine = 'Rcpp', eval = FALSE}
parallelReduce(0, x.length(), obj);
```
...
Gau(const Gau& obj, Split) : input(obj.input), m(obj.m), s(obj.s), value(0) { }
```
and will be called by `parallelReduce`. Note that it takes a reference to an object of type `Gau` as input, and it uses its member (e.g. `obj.input`) to initialise its own members. The `Split` argument is used by `parallelReduce` to split the computation. Then, we have the operator:
```{r engine = 'Rcpp', eval = FALSE}
void operator()(std::size_t begin, std::size_t end) {
 auto gld = [m1 = m, s1 = s](const double x1, const double x2) { 
  return x1 - (x2-m1) * (x2-m1) / (s1*s1*2.0); 
...
```
which is used to perform the evaluation and accumulation of the log-likelihood (as done in the sequential version, above) but only on a subsection of the input vector. Note that we create the lambda function `gld` at this point, with argument `m1` and `s1` fixed to their values (`m` and `s`) within the `Gau` object. Then the join operator:
```{r engine = 'Rcpp', eval = FALSE}
void join(const Gau& rhs) {
    value += rhs.value;
}
```
is used by `parallelReduce` to perform the reduction. It might be possible to accelerate the `dnormPar` further. In fact, if we sustitute 
```{r engine = 'Rcpp', eval = FALSE}
auto gld = [m1 = m, s1 = s]
```
with
