...
addDiffLengh(1, 1:3)
```
does not behave in the way we would expect in `R`. In the first case we are adding `1` to the vector `1:3`, and the output should that no recycling in going on (compare this with $1:3 + 1$ in `R`). In the second case, we are adding `1:3` to a vector of length `1` and `Rcpp` does not do any recycling.
