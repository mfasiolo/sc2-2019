...
```
Depending on the set up of your system (e.g., default linear algebra library used) you might get that the `Rcpp` is a bit slower or faster than the `R` solution, but it is unlikely that you will get massive performance gains. Now, let us consider an `RcppArmadillo` version:
```{r}
sourceCpp(code = '
